...
@App.component('mnoeSubscriptionsList', {
  ...
  controller: ($state, $filter, $log, $uibModal, toastr, MnoeUsers, MnoeCurrentUser, MnoConfirm, MnoeProvisioning) ->
    ...
    fetchSubscriptionEvents = (limit, offset, sort = ctrl.subscriptions.sort) ->
      ctrl.subscriptions.loading = true

      # Add extra filtering
      extra_params = ctrl.filters || {}
      extra_params['where[obsolete]'] = false

      return MnoeProvisioning.getAllSubscriptionEvents(30, 0, null, { 'where[obsolete]': false }).then(
        (response) ->
          ctrl.subscriptions.totalItems = response.headers('x-total-count')
          ctrl.subscriptions.list = response.data
          ctrl.subscriptions.oneAdminLeft = _.filter(response.data, {'admin_role': 'admin'}).length == 1
    ...
    ctrl.subscriptions =
      ...
      pageChangedCb: (nbItems, page) ->
        ...
        ctrl.apiCall(nbItems, ctrl.subscriptions.offset)
    ...
    ctrl.$onInit = ->
      ctrl.apiCall = if ctrl.subscriptionEvent then fetchSubscriptionEvents else fetchSubscriptions
    ...
    ctrl.$onChanges = () ->
      ...
      ctrl.apiCall(ctrl.subscriptions.nbItems, ctrl.subscriptions.offset)
    ...
    ctrl.callServer = (tableState) ->
      ...
      # Call the server
      ctrl.apiCall(ctrl.subscriptions.nbItems, ctrl.subscriptions.offset, sort)
